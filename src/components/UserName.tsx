...
export default function UserName({ className = '', fallback = 'User' }: UserNameProps) {
  ...
  const fetchUserName = async () => {
    try {
      ...
      if (!user) {
        if (shouldLog()) {
          console.log('⚠️ No user session found, using fallback name');
        }
      ...
      if (shouldLog()) {
        console.log('🔍 Fetching user name for user:', user);
      }
  ...
  if (isLoading) {
    return (
      <span className={`${className} animate-pulse bg-gray-300 rounded w-24 h-6`}>
        &nbsp;
      </span>
    );
  }
